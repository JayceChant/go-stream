...
| `interface Collector<T,A,R>` | `collector.Collector[T,A,R]` 接口（四方法；各收集器为非导出具体类型实现，独立子包） | 接口保证行为只读，防止外部改写内部逻辑；收集器族与引擎零耦合，独立成 `stream/collector` 子包（数值约束下沉 `stream/constraints` 叶子包后回归零根包依赖） |
