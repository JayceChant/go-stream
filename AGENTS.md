...
- 积极使用**泛型方法**（方法级类型参数，如 `Map[U any]`）。方法**自有**类型参数可带任意约束（含 `[K comparable]`，已实测 go1.27.0）；**不得以"方法不能带约束"为由让签名退回 `any` 擦除**。牢记三条硬限制（详证见 spec「Go 1.27 泛型方法的关键约束」）：
  1. **接口方法不得声明类型参数**——对外 API 挂在具体类型 `*Stream[T]` 上；
  2. **方法不得约束接收者已有的类型参数**（`func (s *Stream[T comparable])` 语法非法）——需约束 `T` 本身的 API（Distinct/Contains）只能包级函数；
  3. **方法返回 T 的派生类型**（如 `*Stream[[]T]`）触发实例化循环——Chunk/Enumerate 只能包级函数。
